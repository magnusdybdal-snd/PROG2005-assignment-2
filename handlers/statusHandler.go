...
func StatusHandler(w http.ResponseWriter, r *http.Request) {
	...
	status := utils.Status{
		Countries_api:   getHttpCode(utils.RESTCountriesAPI+"no", ctx),
		Metro_api:       getHttpCode(MetroUrl, ctx),
		Currency_api:    getHttpCode(utils.CurrencyAPI+"NOK", ctx),
		Notification_db: getFireStoreCode(ctx),
		Webhooks:        getAmmountWebhooks(ctx),
...
func getAmmountWebhooks(ctx context.Context) int {
...
func getFireStoreCode(ctx context.Context) int {
...
func getHttpCode(url string, ctx context.Context) int {
	client := http.Client{}
